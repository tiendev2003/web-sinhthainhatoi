...
function AdminNotificationPage() {
    ...
    const handleMarkAllAsRead = async () => {
        const response = await axios.put("api/adminNotification/mark-all-as-read");
    ...
    useEffect(() => {
        async function getUnreadNotifications() {
            const unreadNotifcations = await axios.get("api/adminNotification/unread");
            setUnreadNotifications(unreadNotifcations.data);
        }

        async function getAllNotifications() {
            const allNotifications = await axios.get("api/adminNotification/list");
    ...
    const handleDelete = async (id) => {
        console.log(id);
        const response = await axios.delete(`api/adminNotification/delete/${id}`);
        if (response.status === 200) {
            setAlert({
                open: true,
                                ...
                                unreadNotifications.map((item, index) => {
                                    ...
                                    return (
                                        <NotificationItem key={index}>
                                            ...
                                            <NotificationMenu notificationId={item._id} notificationType={"admin"} />
                                        </NotificationItem>
                                    );
                                })}
                            {unreadNotifications.length === 0 && (
